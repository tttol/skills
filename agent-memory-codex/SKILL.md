...
**Location:** `.codex/skills/agent-memory-codex/gmemories/`
...
```bash
# 1. List categories
ls .codex/skills/agent-memory-codex/gmemories/

# 2. View all summaries
rg "^summary:" .codex/skills/agent-memory-codex/gmemories/ --no-ignore --hidden

# 3. Search summaries for keyword
rg "^summary:.*keyword" .codex/skills/agent-memory-codex/gmemories/ --no-ignore --hidden -i

# 4. Search by tag
rg "^tags:.*keyword" .codex/skills/agent-memory-codex/gmemories/ --no-ignore --hidden -i

# 5. Full-text search (when summary search isn't enough)
rg "keyword" .codex/skills/agent-memory-codex/gmemories/ --no-ignore --hidden -i
...
```bash
mkdir -p .codex/skills/agent-memory-codex/gmemories/category-name/
# Note: Check if file exists before writing to avoid accidental overwrites
cat > .codex/skills/agent-memory-codex/gmemories/category-name/filename.md << 'EOF'
---
...
- **Delete**: Remove memories that are no longer relevant
  ```bash
  trash .codex/skills/agent-memory-codex/gmemories/category-name/filename.md
  # Remove empty category folders
  rmdir .codex/skills/agent-memory-codex/gmemories/category-name/ 2>/dev/null || true
  ```
